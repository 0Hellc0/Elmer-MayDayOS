...
OSStart
    ...
    ; enable interruptions，然后会触发PendSV
...
PendSV_Handler
    ...
    ; R4-R11保存在当前TCB 指向的栈顶, 8个正好  0x20
    ; store R4 
    STR     R4 , [R0]                ; *R0 = R4;
...
; 这里是继续下面的函数吗，没有return, 上面是保存当前的寄存器到栈里，下面是取出栈里面的数据写到寄存器
; switch to task1
...
OSCtxSw ;OS context switch  触发 pendsv 中断
    PUSH    {R4, R5}                
    LDR     R4, =NVIC_INT_CTRL       ; R4 = NVIC_INT_CTRL
    LDR     R5, =NVIC_PENDSVSET      ; R5 = NVIC_PENDSVSET
    STR     R5, [R4]                 ; *R4 = R5
